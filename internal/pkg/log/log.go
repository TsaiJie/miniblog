...
func NewLogger(opts *Options) *zapLogger {
	...
	// 将文本格式的日志级别，例如info转化为zapcore.Level类型以供后面使用
	...
	// 自定义 MessageKey 为 message， message语义更明确
	encoderConfig.MessageKey = "message"
	// 自定义 TimeKey 为timestamp， timestamp语义更明确
	encoderConfig.TimeKey = "timestamp"
	// 指定时间序列化函数，将时间序列化为`2006-01-02 15:04:05.000`格式更易读
	encoderConfig.EncodeTime = func(time time.Time, encoder zapcore.PrimitiveArrayEncoder) {
		encoder.AppendString(time.Format("2006-01-02 15:04:05.000"))
	}
	// 指定time.Duration序列化函数，将time.Duration 序列化为经过的毫秒数的浮点数
	// 毫秒数比默认的秒数更精确
	...
	// 把标准库的log.Logger的info级别输出重定向到zap.Logger
	zap.RedirectStdLog(z)
	return logger
}

// Sync Sync调用底层 zap.Logger 的Sync方法，将缓存中的日志刷新到磁盘文件中，主程序需要在退出前调用Sync
func Sync() {
	std.Sync()
}
